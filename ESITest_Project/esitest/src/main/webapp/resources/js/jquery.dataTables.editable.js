...
(function($) {
	...
	$.fn.makeEditable = function(options) {
		...
		function fnGetCellID(cell) {
			...
			// <tr> that that surround the cell (<td> tag). E.g.:
			// /<tr id="17">
			// / <td>...</td><td>...</td><td>...</td><td>...</td>
			// /</tr>
			// /</summary>
			// /<param name="cell" type="DOM" domElement="true">TD cell
			// refference</param>
		...
		function _fnSetRowIDInAttribute(row, id, overwrite) {
			// /<summary>
			// /Utility function used to set id of the row. Usually when a new
			// record is created, added to the table,
			// /and when id of the record is retrieved from the server-side.
			// /It is assumed that id is placed as an id attribute of <tr> that
			// that surround the cell (<td> tag). E.g.:
			// /<tr id="17">
			// / <td>...</td><td>...</td><td>...</td><td>...</td>
			// /</tr>
			// /This function is used when a datatable is configured in the
			// server side processing mode or ajax source mode
			// /</summary>
			// /<param name="row" type="DOM" domElement="true">TR row where
			// record is placed</param>
		...
		function _fnGetRowIDFromAttribute(row) {
			...
			// /</tr>
			// /This function is used when a datatable is configured in the
			// standard client side mode
			// /</summary>
			// /<param name="row" type="DOM" domElement="true">TR row where
			// record is placed</param>
			// /<returns type="Number">Id of the row - by default id attribute
			// placed in the TR tag</returns>
		...
		function _fnSetRowIDInFirstCell(row, id) {
			...
			// record is created, added to the table,
			// /and when id of the record is retrieved from the server-side).
			// /It is assumed that id is placed as a value of the first
			// &lt;TD&gt; cell in the &lt;TR&gt;. As example:
			// /<tr>
		...
		function _fnShowError(errorText, action) {
			...
			// /</summary>
			// /<param name="errorText" type="String">text that should be
			// shown</param>
			// /<param name="action" type="String"> action that was executed
			// when error occured e.g. "update", "delete", or "add"</param>
		...
		function _fnStartProcessingMode() {
			// /<summary>
			// /Function that starts "Processing" mode i.e. shows
			// "Processing..." dialog while some action is executing(Default
			// function)
			// /</summary>
		...
		function _fnEndProcessingMode() {
			// /<summary>
			// /Function that ends the "Processing" mode and returns the table
			// in the normal state(Default function)
			// /It shows processing message only if bProcessing setting is set
			// to true
			// /</summary>
		...
		function fnApplyEditable(aoNodes) {
			...
			// /</summary>
			// /<param name="aoNodes" type="Array[TR]">Aray of table rows
			// &lt;TR&gt; that should be initialized with editable
			// plugin</param>
			...
			var oDefaultEditableSettings = {
				...
				"onsubmit" : function(settings, original) {
					...
					if (settings.type == "text" || settings.type == "select"
							|| settings.type == "textarea") {
						var input = $("input,select,textarea", this);
						sNewCellValue = $("input,select,textarea", $(this))
								.val();
						if (input.length == 1) {
						...
						if (!properties.fnOnEditing(input, settings,
								original.revert, fnGetCellID(original)))
							return false;
						var x = settings;
				...
				"callback" : function(sValue, settings) {
					...
					if (sValue.indexOf(properties.sFailureResponsePrefix) > -1) {
						...
						properties.fnShowError(sValue.replace(
								properties.sFailureResponsePrefix, "").trim(),
								"update");
						status = "failure";
					} else {
						...
						if (properties.sSuccessResponse == "IGNORE"
								|| (properties.aoColumns != null
										&& properties.aoColumns[aPos[2]] != null && properties.aoColumns[aPos[2]].sSuccessResponse == "IGNORE")
								|| (sNewCellValue == null)
							...
							if (sNewCellDisplayValue == null) {
								...
								oTable.fnUpdate(sNewCellDisplayValue, aPos[0],
										aPos[2], bRefreshTable);
							}
							$("td.last-updated-cell", oTable).removeClass(
									"last-updated-cell");
							$(this).addClass("last-updated-cell");
					...
					if (settings.fnOnCellUpdated != null) {
						settings.fnOnCellUpdated(status, sValue, aPos[0],
								aPos[2], settings);
					}
			...
			if (properties.aoColumns != null) {
				...
				for ( var iDTindex = 0, iDTEindex = 0; iDTindex < oSettings.aoColumns.length; iDTindex++) {
					if (oSettings.aoColumns[iDTindex].bVisible) {// if
																	// DataTables
																	// column is
																	// visible
						...
						// 1-indexed array)
						cells = $("td:nth-child(" + (iDTEindex + 1) + ")",
								aoNodes);
						...
						cells.each(function() {
							if (!$(this)
									.hasClass(properties.sReadOnlyCellClass)) {
				...
				cells = $('td:not(.' + properties.sReadOnlyCellClass + ')',
						aoNodes);
				cells
						.editable(properties.sUpdateURL, $.extend({},
								oDefaultEditableSettings,
								properties.oEditableSettings));
			}
		}
		...
		function fnOnRowAdding(event) {
			// /<summary>
			// /Event handler called when a user click on the submit button in
			// the "Add new row" form.
			// /</summary>
		...
		function _fnOnNewRowPosted(data) {
			// /<summary>Callback function called BEFORE a new record is posted
		...
		function fnOnRowAdded(data) {
			// /<summary>
			// /Function that is called when a new row is added, and Ajax
			// response is returned from server
			// / This function takes data from the add form and adds them into
			// the table.
			// /</summary>
			// /<param name="data" type="int">Id of the new row that is returned
			...
			if (properties.fnOnNewRowPosted(data)) {
				...
				if (!oSettings.oFeatures.bServerSide) {
					...
					$("tr.last-added-row", oTable)
							.removeClass("last-added-row");
				...
				if (properties.bUseKeyTable) {
					var keys = oTable.keys;
					/*
					 * Unblock KeyTable, but only after this 'esc' key event has
					 * finished. Otherwise it will 'esc' KeyTable as well
					 */
					setTimeout(function() {
						keys.block = false;
					}, 0);
		...
		function fnOnCancelRowAdding(event) {
			...
			// button in the add new row form
			// /This function clean the add form and error messages if some of
			// them are shown
			// /</summary>
			// /<param name="event" type="int">DOM event that caused an
			// error</param>
		...
		function _fnOnRowDelete(event) {
			...
			if (!properties.bUseKeyTable) {
				...
				nSelectedCell = $('tr.' + properties.sSelectedRowClass + ' td',
						oTable)[0];
			} else {
		...
		function _fnOnDeleting(tr, id, fnDeleteRow) {
			...
			// /Function can be overriden via plugin properties in order to
			// create custom delete functionality
			// /in that case call fnDeleteRow with parameter id, and return
			// false to prevent double delete action
			// /</summary>
			// /<param name="tr" type="JQuery">JQuery wrapper around the TR tag
			// that will be deleted</param>
			// /<param name="id" type="String">Id of the record that wil be
			// deleted</param>
			// /<param name="fnDeleteRow" type="Function(id)">Function that will
			// be called to delete a row. Default - fnDeleteRow(id)</param>
		...
		function fnDeleteRow(id, sDeleteURL) {
			// /<summary>
			// /Function that deletes a row with an id, using the sDeleteURL
			// server page
			// /</summary>
			// /<param name="id" type="int">Id of the row that will be deleted.
			// Id value is placed in the attribute of the TR tag that will be
			// deleted</param>
			// /<param name="sDeleteURL" type="String">Server URL where delete
			// request will be posted</param>
		...
		function fnOnRowDeleted(response) {
			// /<summary>
			// /Called after the record is deleted on the server (in the ajax
			// success callback)
			// /</summary>
			// /<param name="response" type="String">Response text eturned from
			// the server-side page</param>

			properties.fnEndProcessingMode();
			var oTRSelected = nSelectedRow;
			/*
			 * if (!properties.bUseKeyTable) { oTRSelected = $('tr.' +
			 * properties.sSelectedRowClass, oTable)[0]; } else { oTRSelected =
			 * $("td.focus", oTable)[0].parents("tr")[0]; }
			 */
			if (response == properties.sSuccessResponse || response == "") {
				...
				fnSetDisplayStart();
				if (properties.bUseKeyTable) {
					oTable.keys.fnSetPosition(oKeyTablePosition[0],
							oKeyTablePosition[1]);
				}
		...
		function _fnOnEdited(result, sOldValue, sNewValue, iRowIndex,
		...
		function fnGetActionSettings(sAction) {
			...
			// /<param name="sAction" type="String">The name of the
			// action</param>
			...
			properties.fnShowError("Cannot find action configuration settings",
					sAction);
		}
		...
		function fnPopulateFormWithRowCells(oForm, oTR) {
			...
			// /<param name="oTR" type="DOM">Table Row that will populate
			// data</param>
					...
					.each(
							function() {
								...
								if (rel >= iColumnCount)
									...
									// sCellValue =
									// sCellValue.replace(properties.sIDToken,
									// data);
		...
		function fnTakeRowDataFromFormElements(oForm) {
			// /<summary>Populates row with form elements(This should be nly
			// function that read fom elements from form)</summary>
			...
			$(
					"input:text[rel],input:radio[rel][checked],input:hidden[rel],select[rel],textarea[rel],span.datafield[rel],input:checkbox[rel]",
					oForm)
					.each(
							function() {
								var rel = $(this).attr("rel");
								...
								if (rel >= iColumnCount)
									...
									if (this.nodeName.toLowerCase() == "select"
										...
										if (this.type == "checkbox") {
											if (this.checked)
												sCellValue = (this.value != "on") ? this.value
														: "true";
											else
												sCellValue = (this.value != "on") ? ""
														: "false";
										} else
		...
		function fnSendFormUpdateRequest(nActionForm) {
			...
			// /<param name="nActionForm" type="DOM">Form used to enter
			// data</param>
		...
		function fnUpdateRowOnSuccess(nActionForm) {
			// /<summary>Updates table row using form fields after the ajax
			// success callback is executed</summary>
			// /<param name="nActionForm" type="DOM">Form used to enter
			// data</param>
				...
				.each(function() {
					...
					if (properties.bUseKeyTable) {
						...
						keys.event.action(null, null, function(nCell) {
							if ($(nCell)
								...
								return;
							/*
							 * Block KeyTable from performing any events while
							 * jEditable is in edit mode
							 */
							keys.block = true;
							/*
							 * Dispatch click event to go into edit mode - Saf 4
							 * needs a timeout...
							 */
							setTimeout(function() {
								$(nCell).dblclick();
					...
					if (oAddNewRowForm.length != 0) {
						...
						if (oAddNewRowButton.length != 0) {
							...
							if (oAddNewRowButton.data("add-event-attached") != "true") {
								...
								oAddNewRowButton.data("add-event-attached",
										"true");
							}
						...
						if (properties.oAddNewRowFormValidation != null) {
							oAddNewRowForm
									.validate(properties.oAddNewRowFormValidation);
						}
					} else {
						oAddNewRowForm = null;
					}
					...
					if (oDeleteRowButton.length != 0) {
						if (oDeleteRowButton.data("delete-event-attached") != "true") {
							...
							oDeleteRowButton.data("delete-event-attached",
									"true");
						}
					} else {
					...
					$(oTable).on("click", ".table-action-deletelink",
							_fnOnRowDeleteInline);

					if (!properties.bUseKeyTable) {
						// Set selected class on row that is clicked
						// Enable delete button if row is selected, disable
						// delete button if selected class is removed
						$("tbody", oTable)
								.click(
										function(event) {
						...
						oTable.keys.event.focus(null, null, function(nNode, x,
								y) {
					...
					if (properties.aoTableActions != null) {
						for ( var i = 0; i < properties.aoTableActions.length; i++) {
							...
							if (oActionForm.length != 0) {
								...
								oFormOptions = $
										.extend({}, oTableAction.oFormOptions,
												oFormOptions);
								oActionForm.dialog(oFormOptions);
								oActionForm
										.data("action-options", oTableAction);

								var oActionFormLink = $(".table-action-"
										+ sAction);
								...
								// var oActionSubmitButton = $("#form" + sAction
								// + "Ok", oActionForm);
								// aActionFormButtons.push(oActionSubmitButton);
								var oActionFormCancel = $("#form" + sAction
										+ "Cancel", oActionForm);
								if (oActionFormCancel.length != 0) {
									aActionFormButtons.push(oActionFormCancel);
									oActionFormCancel.click(function() {

										var oActionForm = $(this).parents(
												"form")[0];
										// Clear the validation messages and
										// reset form
										$(oActionForm).validate().resetForm(); // Clears
																				// the
																				// validation
																				// errors
										$(oActionForm)[0].reset();
								...
								// Convert all action form buttons to the JQuery
								// UI buttons
								$("button", oActionForm).button();
								/*
								 * if (aActionFormButtons.length > 0) {
								 * oActionForm.dialog('option', 'buttons',
								 * aActionFormButtons); }
								 */
							...
							// properties.aoTableActions.length; i++)
